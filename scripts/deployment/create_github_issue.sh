...
manage_github_issue() {
    ...
    # Skip Github issue creation if the feature is disabled, e.a. GITHUB_CREATE_ISSUE_FEATURE is not true∂
    if [ "$GITHUB_CREATE_ISSUE_FEATURE" != "true" ]; then
        echo "Skipping creating Github issue, feature is disabled." 1>&2
        exit 0
    # Skip Github issue creation if the branch which the build failed, is not the projects main branch
    elif [ "$PROJECT_RELEASE_BRANCH" != "$TRAVIS_BRANCH" ]; then
        echo "Skipping creating Github issue, current branch $TRAVIS_BRANCH is not the main branch." 1>&2
        exit 0
    # Skip Github creation if the artifact was created and exist
    elif [ -f "$TRAVIS_TAG.zip" ]; then
        echo "Artifact was build successfully, no need to create Github issue. Promoting Artifact." 1>&2
        exit 0
    else
        post_github_issue
    fi
